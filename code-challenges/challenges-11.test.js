...
const count = (target, input) => {
  // Solution code here...
  let counter= input.reduce((x,y)=>
  x+y.reduce((x,y)=> y===target?x+1:x,0),0);
  return counter;
};
...
const totalSum = (input) => {
  let sum = 0;
  input.map(element => { element.map(element => sum = sum + element); });
...
const divisibleByFiveTwoToThePower = (input) => {
      let result = input.map(element => element.filter(number => !(number % 5)))
      let result2 = result.map(e=>e.filter(e=>typeof(e) !== 'string'))
      let result3 = result2.map(e=>e.map(e=>Math.pow(2, e)))
      console.log(result3)
      return result3
};
...
let findMaleAndFemale = (data) => {
  let fmArray = data.filter(element => element.gender === 'male' ||element.gender==='female').map(element => element.name).join(' and ');
...
let findShortest = (data) => {
  ...
  let shortestFirstName = result[0].name
