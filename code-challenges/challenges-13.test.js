...
const firstLetters = (arr) => {
  return arr.map(element => element.slice(0,1))
};
...
const findHappiness = (arr) => {
  let happyArray = arr.filter(element => element.includes(':)') )
...
const standardizePhoneNumbers = (arr) => {
 let x =  arr.map(element => element.replace(/-/g, ""))
 let y = x.map(element => element.replace('(', ""))
 let z = y.map(element => element.replace(')', ""))
 let q = z.map(element => element.replace(' ', ""))
...
const allHappy = (arr) => {
  let x = arr.map(element => element.includes(':)'))
  console.log(x)
  let result;
  if (x.includes(false)) {
    result = false;
